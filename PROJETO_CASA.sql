...
-- Introdução de dados da tabela de referencia nas variaveis
    ...
    -- Loop para contagem de meses faltantes até quitação do valor introduzido
    WHILE V_BANCO < V_CUSTO LOOP
        V_BANCO := V_BANCO+v_acrescimo;
        V_COUNT := V_COUNT +1;
    END LOOP;
    
    -- criação de anos e meses a partir do count
    V_ANOS := V_COUNT/12;
    ...
    -- print's com informações sobre as tabelas
    IF v_count = 1 THEN
    ...
    -- exceção de erro para quando não é encontrado o banco ou o tipo de emprestimo na tabela
