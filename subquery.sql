...
WHERE A.DEPARTMENT_ID=B.DEPARTMENT_ID) AS SOMA_SALARIOS,
(SELECT COUNT(*) FROM HR.EMPLOYEES B
WHERE A.DEPARTMENT_ID=B.DEPARTMENT_ID) AS QNTD_FUNCIONARIOS,
(SELECT AVG(B.SALARY) FROM HR.employees B
WHERE A.DEPARTMENT_ID=B.DEPARTMENT_ID) AS MÉDIA_SALARIAL
FROM HR.DEPARTMENTS A;